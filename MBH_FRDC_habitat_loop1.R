...
for(i in 1:length(allrasters)){
  ...
  # Name and save raster of incluusion probs --
    get.name <- names(inclProbs)
    site.code <- substr(get.name, 6, 8)
    nam <- paste(site.code, "InclProbs_SentFine.tif", sep ='_')
    #nam <- paste(paste('c', i, sep =''), "InclProbs_SentFine.tif", sep ='_')
    writeRaster(inclProbs, paste(o.dir, nam, sep='/'), overwrite=TRUE)
